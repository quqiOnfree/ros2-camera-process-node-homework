...
public:
    ...
    bool add(std::uint8_t number, cv::InputArray src) {
        ...
        auto res = std::ranges::max_element(contours.begin(), contours.end(),
        ...
        auto [iter, bres] = m_number_map.emplace(std::make_pair(number, std::move(hu_moment)));
        return bres;
    }
    ...
    static bool detect(cv::InputArray src, std::vector<cv::Point>& points) {
        ...
        // cv::threshold(blur_img, blur_img, 0, 255, cv::THRESH_BINARY | cv::THRESH_OTSU);
        cv::adaptiveThreshold(blur_img, blur_img, 255, cv::ADAPTIVE_THRESH_GAUSSIAN_C, cv::THRESH_BINARY, 11, 2);
        cv::Canny(blur_img, can_img, 25, 75);
        auto kernel = cv::getStructuringElement(cv::MorphShapes::MORPH_RECT, cv::Size{2, 2});
        ...
        auto res = std::ranges::max_element(range.begin(), range.end(),
            [](const std::vector<cv::Point>& a, const std::vector<cv::Point>& b) {
                return cv::contourArea(a) < cv::contourArea(b);
            });
        if (res == range.end()) {
    ...
    std::int8_t decode(cv::InputArray src, const std::vector<cv::Point>& points, cv::InputOutputArray dst = cv::noArray()) const {
        if (points.size() != 4) {
            return -1;
        }
        cv::Point2f arr[4] = {
...
protected:
    template<class Ty>
    static void reorder(std::span<cv::Point_<Ty>, 4> arr) {
        if (arr.size() != 4) {
            throw std::exception{};
        }
        ...
        tmp[0] = arr[std::distance(sumPoints.begin(), std::min_element(sumPoints.begin(), sumPoints.end()))];
        tmp[1] = arr[std::distance(subPoints.begin(), std::max_element(subPoints.begin(), subPoints.end()))];
        tmp[2] = arr[std::distance(subPoints.begin(), std::min_element(subPoints.begin(), subPoints.end()))];
        tmp[3] = arr[std::distance(sumPoints.begin(), std::max_element(sumPoints.begin(), sumPoints.end()))];
        for (std::size_t i = 0; i < tmp.size(); ++i) {
            arr[i] = std::move(tmp[i]);
        }
    }

    static void findContours(cv::InputArray src, std::vector<std::vector<cv::Point>>& poly_contours) {
        std::vector<std::vector<cv::Point>> contours;
    ...
    static void drawContours(cv::InputArray src, cv::InputOutputArray dst, const std::vector<std::vector<cv::Point>>& contours) {
        for (std::size_t size = contours.size(), i = 0; i < size; ++i) {
            ...
            // 画上轮廓
            cv::drawContours(dst, contours, static_cast<int>(i), cv::Scalar{255, 0, 255}, 2);
